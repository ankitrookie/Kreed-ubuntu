...
const Stumped = ({setModal, setRunEffectTrigger}) => {
  ...
  const handleDeliveryTypeSelection = async deliveryType => {
    let value;
  ...
  const chooseBatsMan = async player => {
    const stumpedPlayerData = getRoomResponse?.data[0]?.scores[0]?.cricScore;
    let currentStricker = null;

    for (const player of stumpedPlayerData) {
      if (player.isStriker) {
        currentStricker = {
    ...
    // TODO: runConsided need to be above 0, to make our ball increase -> backed need to be fixed
    ...
    //     updatePlayerScoreAsync({playerData: stumpedPlayerData}),
    //   );
  ...
  // if it is wide and no-ball, I have to add the runs in the team tally and bowler tally
  // if it is dot ball, then batter out -> true and choose next batsman, and one more wicket added to bowler team tally.
